        ...
        <div className='lg:hidden bg-white shadow-lg fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500 border'>
            {allTypes.map((item) => (
                <Link key={item.id} to={`/${item.id}`} className='w-1/4' >
                    <div className={
                        type === item.id ? "flex text-center flex-col relative items-center text-xl text-zomato-400" : "flex text-center flex-col items-center text-xl"
                    }>
                        <div className={
                            type === item.id ? "w-full h-full border-t-2 p-3 border-zomato-400 flex flex-col items-center":
